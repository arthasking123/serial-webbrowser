...
{   
    assert(portnr > 0 && portnr < MAX_SERIAL_PORT_NUM);   
...
{   
    ...
    DWORD dwError = 0;   
    COMSTAT comstat;   
    // ÒÔÏÂÕâ¶Î,ÒòÎª±ØÐë³õÊ¼»¯,ÒÔ±£Ö¤²»»áÒòÎªVC7ÒÔÉÏ°æ±¾¼ì²é¹ýÑÏÔì³É"Î´³õÊ¼»¯¾ÍÊ¹ÓÃ"´íÎó   
    comstat.cbInQue=comstat.cbOutQue=comstat.fCtsHold=comstat.fDsrHold=comstat.fEof   
        =comstat.fReserved=comstat.fRlsdHold=comstat.fTxim=comstat.fXoffHold   
        =comstat.fXoffSent=0;   
    BOOL  bResult = TRUE;   
...
{   DWORD dwThreadId;
	...
	if (!(m_Thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)CommThread, this, 0, &dwThreadId)))   
        return FALSE;   
...
{   
    ...
    //sprintf_s(Temp, "WARNING:  %s Failed with the following error: \n%s\nPort: %d\n", (char*)ErrorText, lpMsgBuf, m_nPortNr);    
...
{   
	...
	static unsigned char RxBuffArray[50];
    ...
    {    
		...
		RxBuffArray[RxLength] = RXBuff;
		RxLength+=BytesRead;
		if (RxLength > 1)
		{
			if (RxBuffArray[RxLength - 1] == 0x0a && RxBuffArray[RxLength - 2] == 0x0d)
			{
				::SendMessage((port->m_pOwner), WM_COMM_RXCHAR, (WPARAM)RxBuffArray, (LPARAM)port->m_nPortNr);
				RxLength = 0;
			}
		}
